...
9. State/config directories are mode `0700` and files are mode `0600`. A durable state save opens the store-state directory once with no-follow semantics, verifies that the held descriptor is still bound to the expected owner-private directory, creates a new exclusive no-follow temporary entry relative to it, and performs replacement, cleanup, and directory synchronization relative to that same descriptor. The temporary entry and installed target must remain the regular, single-link inode opened by the writer; binding changes fail closed. The file is synchronized before atomic replacement and the directory afterward.
