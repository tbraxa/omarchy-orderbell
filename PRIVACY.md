...
State lives under `${XDG_STATE_HOME:-$HOME/.local/state}/orderbell/`. Runtime locks live under `${XDG_RUNTIME_DIR}/orderbell/`. Directory permissions are `0700` and state files are `0600`. Each state update holds one verified no-follow descriptor for the owner-private store-state directory, creates an exclusive no-follow temporary entry relative to it, synchronizes the file, atomically replaces and cleans up entries relative to the held descriptor, verifies the temporary and installed inode bindings, and synchronizes the directory. A changed directory binding or unsafe entry fails closed.
