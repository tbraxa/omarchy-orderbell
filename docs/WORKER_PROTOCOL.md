...
OrderBell-owned directories are mode `0700`; state and lock files are mode `0600`. State reads are size-bounded and refuse non-regular, wrong-owner, group/world-accessible, or symlinked files. For a write, the worker opens the store-state directory once with `O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC`, verifies through `fstat` and a no-follow path lookup that the descriptor is an owner-private directory still bound to the expected device/inode, and keeps that descriptor until the transaction ends. It validates the existing target relative to the descriptor and creates a random temporary basename there with `O_CREAT | O_EXCL | O_NOFOLLOW | O_CLOEXEC` and mode `0600`. After writing, it synchronizes the open file, then revalidates the target, directory binding, and temporary entry's type, owner, link count, mode, device/inode, and size against the still-open file descriptor. Replacement uses the held descriptor for both source and destination; the installed target must resolve without following links to that same written inode before and after descriptor-relative directory `fsync`. Failure cleanup unlinks only the temporary basename relative to the held descriptor. A zero-byte write or any binding/type/ownership verification failure aborts without following an attacker-selected path. One non-blocking `fcntl` lock serializes poll, authentication, and local state actions per store.

The descriptor-relative transaction prevents a pathname-component replacement from redirecting an in-progress save after the verified directory descriptor is opened. It does not treat a fully hostile same-user process as a security boundary. A failure reported after atomic replacement or during directory synchronization means a new directory entry may already be visible even though its identity or crash durability was not accepted; the caller must not assume that either the previous or intended state remains and a later invocation revalidates the on-disk state before use.
