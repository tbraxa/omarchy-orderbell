...
| Concurrent workers corrupt state or duplicate | Atomic per-store runtime lock, atomic replacement, owner-only paths | A hostile same-user process can remove locks or tamper with state |
| Symlink/hard-link/path attack overwrites another file | Fixed XDG root, hash-based filenames, `lstat`/no-follow checks, refuse symlinked roots/targets, owner modes, atomic same-directory replace | Same-user attackers are not a strong privilege boundary; platform filesystem semantics still apply |
